function searchInJson(year){
      var data = getJSON()
      // d3.json("/Resources/results2.json", function(data) {
      //   console.log(data[0]);
      // });
        ...
        if ((year >= 1903 && year < 1915) || (year > 1918 && year < 1940) || (year > 1946 && year <= 2017))  {
            if (data[year].Winner.name == 'd\u00e9sattribu\u00e9'){
                ...
                $('#winner').html('<a class="img col-xs-12"> Gagnant du Tour de France : <b>' + data[year].Winner.name + '</b> (' + data[year].Winner.nationality + ')</a>');
            if (data[year].Winner.name != 'none' && data[year].Climber.nationality != "none")
                $('#climber').html('Meilleur grimpeur : <b>' + data[year].Climber.name + '</b> (' + data[year].Climber.nationality + ')');
            if (year < 1953)
                ...
                $('#sprinter').html('Meilleur sprinteur : <b>' + data[year].Sprinter.name + '</b> (' + data[year].Sprinter.nationality + ')');
            $('#winner').prepend('<img class="img col-xs-1" src="./Resources/img/MAILLOTJAUNE.png" />')
        }
